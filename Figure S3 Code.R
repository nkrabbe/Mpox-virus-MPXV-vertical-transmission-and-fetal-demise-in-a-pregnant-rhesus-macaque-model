...
placenta_titers_figure <-
  ...
  theme_classic() +
  scale_y_log10(breaks= trans_breaks('log10', function(x) 10^x), labels= trans_format('log10', math_format(10^.x)), expand= c(0,0), limits= c(1,10^8), name= 'Virus Titers (PFU/g)') +
  ...
  labs(title='Placental Tissue') +
  theme(plot.title = element_text(hjust= 0.5, face= 'bold'), aspect.ratio= 1, axis.text = element_text(size=11), axis.title = element_text(size= 11)) 
placenta_titers_figure
...
af_titers_figure <-
  ...
  theme_classic() +
  scale_y_log10(breaks= trans_breaks('log10', function(x) 10^x), labels= trans_format('log10', math_format(10^.x)), expand= c(0,0), limits= c(1,10^8), name= 'Virus Titers (PFU/mL)') +
  scale_x_discrete(name='ID') +
  scale_color_manual(values = c('#374e55', '#79af97'), breaks= c('101', '102')) +
  scale_fill_manual(values = c('#374e55', '#79af97'), breaks= c('101', '102')) +
  guides(color = guide_legend('ID'), fill= guide_legend('ID')) +
  labs(title="Amniotic Fluid") +
  theme(plot.title = element_text(hjust= 0.5, face= 'bold'), aspect.ratio= 1, axis.text = element_text(size=11), axis.title = element_text(size=11)) +
...
soni_comparison_figure <-
  ...
  scale_y_log10(name= 'Virus Titers (PFU/mL)', breaks= trans_breaks('log10', function(x) 10^x), labels = trans_format('log10', math_format(10^.x)), expand = c(0,0), limits = c(1,10^6)) +
  ...
  labs(title = 'Skin Lesion') +
  theme(aspect.ratio = 1, plot.title= element_text(face= 'bold', hjust= 0.5), axis.text = element_text(size= 10), axis.title = element_text(size=11)) + guides(color= 'none', fill= 'none') + 
...
tiff('Figure S3.tiff', units='cm', height=8, width=18, res=600)
